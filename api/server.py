...
# # Optionally preload whisper model at startup to make /health show available
# PRELOAD_WHISPER = os.getenv("PRELOAD_WHISPER", "false").lower() in ("1", "true", "yes")

# # CORS configuration for web and Electron
# # Allow common local development origins and file:// / null (Electron)
# # Use a permissive policy for local development; tighten for production.
# CORS(app, resources={r"/api/*": {"origins": [
#     "http://localhost:5173",
#     "http://localhost:5000",
#     "http://127.0.0.1:5000",
...
# # Whisper model (lazy) using openai-whisper
# whisper_model = None
# hf_client = None
# # In-memory store for last transcription (useful for clients to poll)
# last_transcription: Dict[str, Any] = {
#     'text': None,
#     'bible_references': [],
#     'verses': [],
...
#     Expected reference formats: "John 3:16", "Psalm 23", "1 Corinthians 13:4-7".
#     Returns a dict with keys similar to the previous implementation or None on failure.
#     """
#     try:
#         # locate bible.db relative to this file
#         base_dir = os.path.dirname(__file__)
#         db_path = os.path.normpath(os.path.join(base_dir, '..', 'src', 'data', 'bible.db'))
#         if not os.path.exists(db_path):
#             logger.error(f"Bible DB not found at {db_path}")
#             return None

#         conn = sqlite3.connect(db_path)
#         cursor = conn.cursor()

#         # Parse reference like 'Book Chapter:Verse' or chapter-only
#         m = re.match(r'^\s*([123]?\s*[A-Za-z ]+)\s*(\d+)(?::(\d+)(?:-(\d+))?)?\s*$', reference)
#         if not m:
#             # Fallback: do a LIKE search on the text
#             cursor.execute(
#                 "SELECT book, chapter, verse, text FROM bible WHERE version=? AND text LIKE ? LIMIT 10",
#                 (os.getenv('DEFAULT_BIBLE_VERSION', 'KJV66'), f"%{reference}%"),
#             )
#             rows = cursor.fetchall()
#             conn.close()
#             if not rows:
...
#                 'verse': rows[0][2],
#                 'text': text.strip(),
#                 'translation': os.getenv('DEFAULT_BIBLE_VERSION', 'KJV66'),
#             }

#         book = m.group(1).strip()
#         chapter = int(m.group(2))
#         verse_start = int(m.group(3)) if m.group(3) else None
#         verse_end = int(m.group(4)) if m.group(4) else (verse_start if verse_start else None)

#         version = os.getenv('DEFAULT_BIBLE_VERSION', 'KJV66')

#         if verse_start:
#             cursor.execute(
#                 "SELECT text, book, chapter, verse FROM bible WHERE version=? AND book=? AND chapter=? AND verse BETWEEN ? AND ? ORDER BY verse",
#                 (version, book, chapter, verse_start, verse_end),
#             )
#         else:
#             # whole chapter
#             cursor.execute(
#                 "SELECT text, book, chapter, verse FROM bible WHERE version=? AND book=? AND chapter=? ORDER BY verse",
...
#         verse_ref = f"{book} {chapter}" + (f":{verse_start}" if verse_start else "")
#         return {
#             'id': f"bible-{hash(verse_ref)}",
#             'reference': verse_ref,
#             'book': book,
#             'chapter': chapter,
#             'verse': verse_start or 0,
#             'text': text.strip(),
#             'translation': version,
#         }
#     except Exception as e:
#         logger.error(f"Error fetching Bible verse from DB: {e}")
#         return None
...
#                     verses.append(verse_data)
#         else:
#             # If no explicit reference, perform a simple local text search in bible.db
#             # Use the DEFAULT_BIBLE_VERSION env var or KJV
...
# # NOTE: Removed GET /api/last-transcription endpoints to simplify the flow.
# # Clients should POST audio to /api/transcribe and use the POST response directly.
...
# if __name__ == '__main__':
#     logger.info("Starting Bible Echo Python Backend (openai-whisper)...")
#     port = int(os.environ.get('PORT', 5000))
#     if PRELOAD_WHISPER:
#         initialize_whisper_model()
#     logger.info(f"Server starting on port {port}")
